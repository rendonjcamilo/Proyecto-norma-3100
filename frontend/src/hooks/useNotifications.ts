...
export function useNotifications(): UseNotificationsReturn {
  ...
  const apiRef = useRef<AxiosInstance>(axios.create({ baseURL: API_URL }));
  ...
  const connect = useCallback(
    (userId: string, providerId: string, role: string) => {
      ...
      try {
        ...
        socketRef.current.on('notification', (event: any) => {
  ...
  const acknowledge = useCallback(async (notificationId: string) => {
    try {
      ...
      // Update local state
      setNotifications(prev =>
        prev.map(n => (n.id === notificationId ? { ...n, isAcknowledged: true } : n))
      );
    } catch (err) {
