...
describe('ProgressUpdateForm', () => {
    ...
    it('should render form with followups', () => {
        render(_jsx(ProgressUpdateForm, { actionId: "act-1", followups: mockFollowups, onSubmit: mockOnSubmit, onCancel: mockOnCancel }));
    ...
    it('should have default followup selected', () => {
        render(_jsx(ProgressUpdateForm, { actionId: "act-1", followups: mockFollowups, onSubmit: mockOnSubmit, onCancel: mockOnCancel }));
        const select = screen.getByDisplayValue('Paso 1: Planificación (pendiente)');
    ...
    it('should accept evidence URL', () => {
        ...
        expect(evidenceInput.value).toBe('https://example.com/test.pdf');
    });
    it('should accept comment', () => {
        render(_jsx(ProgressUpdateForm, { actionId: "act-1", followups: mockFollowups, onSubmit: mockOnSubmit, onCancel: mockOnCancel }));
        const commentInput = screen.getByPlaceholderText('Describe qué se ha completado y qué está pendiente...');
        fireEvent.change(commentInput, { target: { value: 'Progreso significativo realizado' } });
        expect(commentInput.value).toBe('Progreso significativo realizado');
    });
    ...
    it('should show loading state', () => {
        render(_jsx(ProgressUpdateForm, { actionId: "act-1", followups: mockFollowups, onSubmit: mockOnSubmit, onCancel: mockOnCancel, loading: true }));
        expect(screen.getByText('Guardando...')).toBeInTheDocument();
    });
    it('should validate completion percentage range', async () => {
        const mockOnSubmitWithValidation = vi.fn().mockRejectedValue(new Error('El progreso debe estar entre 0 y 100%'));
        render(_jsx(ProgressUpdateForm, { actionId: "act-1", followups: mockFollowups, onSubmit: mockOnSubmitWithValidation, onCancel: mockOnCancel }));
        const rangeInput = screen.getByRole('slider');
...
//# sourceMappingURL=ProgressUpdateForm.test.js.map