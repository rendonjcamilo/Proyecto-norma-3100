...
describe('LocationSelector', () => {
    it('renders select element', () => {
        ...
        const { container } = render(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: "1", onChange: mockChange }));
        const select = container.querySelector('select');
        expect(select).toBeInTheDocument();
    });
    it('displays all locations as options', () => {
        const mockChange = jest.fn();
        render(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: "1", onChange: mockChange }));
    ...
    it('displays placeholder option', () => {
        const mockChange = jest.fn();
        render(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: null, onChange: mockChange }));
    ...
    it('displays selected location', () => {
        const mockChange = jest.fn();
        const { rerender } = render(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: "2", onChange: mockChange }));
        const select = screen.getByDisplayValue('Hospital Norte (Medellín)');
        expect(select).toBeInTheDocument();
    });
    it('handles empty locations list', () => {
        const mockChange = jest.fn();
        render(_jsx(LocationSelector, { locations: [], selectedLocationId: null, onChange: mockChange }));
    ...
    it('updates when locations prop changes', () => {
        const mockChange = jest.fn();
        const { rerender } = render(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: "1", onChange: mockChange }));
        const newLocations = [
            ...mockLocations,
            { id: '4', name: 'Clínica Nueva', city: 'Cartagena', state: 'Bolívar' },
        ];
        rerender(_jsx(LocationSelector, { locations: newLocations, selectedLocationId: "1", onChange: mockChange }));
        expect(screen.getByDisplayValue('Clínica Nueva (Cartagena)')).toBeInTheDocument();
    });
    it('updates when selectedLocationId prop changes', () => {
        const mockChange = jest.fn();
        const { rerender } = render(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: "1", onChange: mockChange }));
        rerender(_jsx(LocationSelector, { locations: mockLocations, selectedLocationId: "3", onChange: mockChange }));
    ...
    it('preserves location data structure', () => {
        ...
        render(_jsx(LocationSelector, { locations: [testLocation], selectedLocationId: "test-id-123", onChange: mockChange }));
...
//# sourceMappingURL=LocationSelector.test.js.map