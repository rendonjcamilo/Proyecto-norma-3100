...
describe('Notification System - E2E Workflows', () => {
    ...
    describe('Complete Notification Workflow', () => {
        it('should create and deliver notification through full system', async () => {
            ...
            // 4. Mark as read
            const readResponse = await axios.put(`${API_URL}/notifications/${notificationId}/read`, {}, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
            ...
            const ackResponse = await axios.put(`${API_URL}/notifications/${notificationId}/acknowledge`, {}, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
            ...
            const deleteResponse = await axios.delete(`${API_URL}/notifications/${notificationId}`, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
        ...
        it('should handle preference management workflow', async () => {
            ...
            const getPrefsResponse = await axios.get(`${API_URL}/notifications/preferences`, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
            ...
            const updateResponse = await axios.put(`${API_URL}/notifications/preferences`, newPrefs, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
            ...
            const verifyResponse = await axios.get(`${API_URL}/notifications/preferences`, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
        ...
        it('should handle statistics retrieval', async () => {
            ...
            const dashboardResponse = await axios.get(`${API_URL}/notifications/stats/dashboard`);
            expect(dashboardResponse.status).toBe(200);
            expect(dashboardResponse.data).toBeDefined();
            // 2. Get performance metrics
            const perfResponse = await axios.get(`${API_URL}/notifications/stats/performance`);
    ...
    describe('Error Handling Workflows', () => {
        ...
        it('should handle concurrent requests', async () => {
            const requests = Array(5)
                .fill(null)
                .map(() => axios.get(`${API_URL}/notifications/unread/count`, {
                headers: {
                    'x-user-id': testUser.userId,
                    'x-provider-id': testUser.providerId,
                },
