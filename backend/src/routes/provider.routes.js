...
export function createProviderRouter(pool, eventStore) {
    const router = Router();
     ...
     */
    router.post('/providers', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
        try {
            const { rut, legal_name, trade_name, legal_entity_type, address, city, department, country, status } = req.body;
    ...
    router.get('/providers', authMiddleware, async (req, res) => {
        try {
            ...
            const filters = {
                ...
                created_after: req.query.created_after ? new Date(req.query.created_after) : undefined,
                created_before: req.query.created_before ? new Date(req.query.created_before) : undefined,
            };
            let providers = [];
            if (role === 'super_admin') {
                ...
                const result = await pool.query(`SELECT p.* FROM providers p
             INNER JOIN auditor_providers ap ON p.id = ap.provider_id
             WHERE ap.auditor_id = $1`, [user?.user_id]);
                providers = result.rows;
            }
            else if (role === 'provider_admin') {
                // Provider admin sees only their own provider
                const result = await pool.query(`SELECT * FROM providers WHERE id = $1`, [user?.provider_id]);
                providers = result.rows;
            }
            res.json({
     ...
     */
    router.get('/providers/:id', authMiddleware, async (req, res) => {
        try {
            ...
            if (user.role === 'auditor') {
                const result = await pool.query('SELECT 1 FROM auditor_providers WHERE auditor_id = $1 AND provider_id = $2', [user.user_id || user.id, provider.id]);
     ...
     */
    router.put('/providers/:id/status', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
        try {
     ...
     */
    router.get('/providers/:id/locations', authMiddleware, async (req, res) => {
        try {
            const provider = await providerModel.getProviderById(req.params.id);
            if (!provider) {
                return res.status(404).json({ error: 'Provider not found' });
     ...
     */
    router.post('/providers/:providerId/assign-auditor', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
        try {
            ...
            const auditorResult = await pool.query(`SELECT id FROM users WHERE id = $1 AND role = 'auditor'`, [auditorId]);
            if (auditorResult.rows.length === 0) {
                return res.status(404).json({ error: 'Auditor not found' });
            }
            // Assign auditor to provider
            await pool.query(`INSERT INTO auditor_providers (auditor_id, provider_id, assigned_by)
           VALUES ($1, $2, $3)
           ON CONFLICT (auditor_id, provider_id) DO NOTHING`, [auditorId, req.params.providerId, user?.user_id]);
            logger.info({
     ...
     */
    router.delete('/providers/:providerId/auditors/:auditorId', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
        try {
            const user = req.user;
            // Verify provider exists
            const provider = await providerModel.getProviderById(req.params.providerId);
            ...
            const result = await pool.query(`DELETE FROM auditor_providers
           WHERE auditor_id = $1 AND provider_id = $2
           RETURNING *`, [req.params.auditorId, req.params.providerId]);
            if (result.rows.length === 0) {
     ...
     */
    router.get('/providers/:providerId/auditors', authMiddleware, async (req, res) => {
        try {
            // Verify provider exists
            const provider = await providerModel.getProviderById(req.params.providerId);
            if (!provider) {
            ...
            const result = await pool.query(`SELECT u.id, u.email, u.first_name, u.last_name, ap.assigned_at, ap.assigned_by
           FROM users u
           INNER JOIN auditor_providers ap ON u.id = ap.auditor_id
           WHERE ap.provider_id = $1
           ORDER BY ap.assigned_at DESC`, [req.params.providerId]);
            res.json({
                provider_id: req.params.providerId,
