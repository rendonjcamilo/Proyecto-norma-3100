 ...
 */
export function providerAccessMiddleware(pool, providerIdKeys = ['provider_id', 'providerId']) {
    return async (req, res, next) => {
        try {
            ...
            if (userRole === 'provider_admin') {
                const result = await pool.query('SELECT provider_id FROM users WHERE id = $1', [userId]);
            ...
            // auditor: can only access assigned providers
            if (userRole === 'auditor') {
                const result = await pool.query('SELECT 1 FROM auditor_providers WHERE auditor_id = $1 AND provider_id = $2', [userId, providerId]);
                if (result.rows.length === 0) {
                    logger.warn({
